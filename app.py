...
# ─────────────────────────────────────────────
# CUSTOM CSS (unchanged — your premium UI)
# ─────────────────────────────────────────────
st.markdown("""
<style>
/* ===== GLOBAL ===== */
...
def _escape(text: str) -> str:
    """HTML-escape to prevent injection in unsafe_allow_html blocks."""
...
def conf_bar_color(score: float) -> str:
    """Return CSS color for a confidence bar fill."""
    if score >= 0.90:
        return "#22c55e"
    elif score >= 0.75:
        return "#f59e0b"
...
def build_fusion_query(
    ...
    """
    Build a combined search query using fusion logic.
 
    - If image is trusted (weight > 0.3), append the normalized label to symptoms.
    - If image is weak (weight <= 0.3), use symptoms only.
    - Never let image label REPLACE symptom text.
    """
    ...
    if best and image_weight > 0.30:
        ...
        if image_label.lower() not in ("healthy", "background", ""):
            # Append image label but don't let it dominate
...
def rerank_results(results: list, image_result: dict, image_weight: float) -> list:
    """
    Re-rank retrieval results using combined evidence scoring.
 
    Combined score = (1 - image_weight) * retrieval_score + image_weight * image_boost
 
    image_boost: 1.0 if the result name matches the image prediction, 0.0 otherwise.
    """
    ...
    for r in results:
        ...
        # Check if image prediction matches this result
        image_boost = 0.0
...
def determine_evidence_source(image_result: dict, image_weight: float, has_symptoms: bool) -> str:
    """Return a human-readable string about which evidence source dominated."""
...
if run_check:
    ...
    # ── STEP 1: Image prediction (strict) ──
    image_result = {
        "predictions": [],
    ...
    # ── STEP 4: Retrieve matches from dataset ──
    ...
    # ── STEP 5: Re-rank using fusion scoring ──
    results = rerank_results(results, image_result, image_weight)
 
    top_result = results[0]
    other_results = results[1:]
 
    # ── STEP 6: Evidence source determination ──
    ...
    with left_col:
        ...
        # ── Show image model predictions with confidence bars ──
        ...
        # ── Management Recommendations card ──
        st.markdown('<div class="dashboard-card">', unsafe_allow_html=True)
        ...
        with m2:
            st.markdown(
                ...
                <div class="metric-card">
                    ...
                    <div class="metric-sub">Estimated treatment cost</div>
                </div>
                """,
                unsafe_allow_html=True
            )
    ...
    # ─────────────────────────────────────────
    # OTHER POSSIBLE MATCHES (properly ranked)
    # ─────────────────────────────────────────
    if other_results:
        st.markdown(
